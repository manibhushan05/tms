...
sudo aws s3 sync "$script_dir"/../../../transiq/static/ s3://aahodocuments/static/ --region ap-south-1 --delete
sudo aws cloudfront create-invalidation --distribution-id E3SJREZ32KJGEQ --path /static/aaho/*
...
aws autoscaling create-launch-configuration --launch-configuration-name StageLC_$(cat ami.json | jq '.ImageId' | sed 's/"//g') --key-name aahostageserver --image-id $(cat ami.json | jq '.ImageId' | sed 's/"//g') --instance-type t2.small --security-groups sg-8234aee9 --iam-instance-profile S3-Admin-Access --instance-monitoring Enabled=false --block-device-mappings "[{\"DeviceName\":\"/dev/sda1\",\"Ebs\":{\"VolumeSize\":100}}]" --region ap-south-1
