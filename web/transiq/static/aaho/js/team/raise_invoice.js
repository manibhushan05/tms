...
var raise_invoice_dt = $('#raise_invoice_DataTable').DataTable({
    "serverSide": true,
    "processing": true,
    "DisplayLength": 25,
    dom: 'Bfrtip',
    ...
    buttons: [
        'pageLength', {
            extend: 'excelHtml5',
            title: 'Generate raise invoice List ' + date.toLocaleTimeString("en-US", options)
        },
        {
            extend: 'csvHtml5',
            title: 'Generate raise invoice List ' + date.toLocaleTimeString("en-US", options)
    ...
    "ajax": {
        "type": "GET",
        "serverSide": true,
        "url": "/api/team-manual-booking-list/?booking_data_category=raise_invoice&format=datatables",
        ...
        dataSrc: function (json) {
            if (json.summary) {
                $('.overdueCount').text(json.summary.overdue);
                $('.pendingCount').text(json.summary.pending);
            }
            return json.data;
        },
    ...
    "drawCallback": function (settings, json) {
        if (settings.json.summary) {
            $.each(settings.json.summary.overdue_bookings_list, function (index, value) {
                $('a[data-id=' + value + ']').css("background-color", "#8B0000");
            });
        }
    }
});
