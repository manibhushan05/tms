...
public class CityParser {
    ...
    public ArrayList<CityParser> getArrayListFromJson(JSONArray jsonArray) {
        ArrayList<CityParser> cityList = new ArrayList<>();
        if (jsonArray != null && jsonArray.length() > 0) {
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                try {
                    ...
                    if (city != null) {
                        ...
                        String name = Utils.get(city, "name");
                        cityParser.setId(id);
                        cityParser.setText(name);
                        cityList.add(cityParser);
                    }
    ...
    public ArrayList<CityParser> getCitySuggestionData(String searchQuery) {
        ...
        try {
            URL js = new URL(in.aaho.android.employee.requests.GetCityDataRequest
