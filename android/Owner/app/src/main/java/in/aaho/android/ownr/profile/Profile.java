...
import in.aaho.android.ownr.booking.UserAddress;
...
public class Profile {
    ...
    public static final String CITY_ID_KEY = "city";
    ...
    public static Profile fromJson(JSONObject user) throws JSONException {
        ...
        profile.userFullName = nullToBlank(user.getString("name"));
        profile.userContactName = nullToBlank(user.getString("contact_person_name"));
        ...
        if(user.has(CITY_KEY)) {
            JSONObject city = user.getJSONObject(CITY_KEY);
            profile.mCityId = city.optString(CITY_ID_KEY);
            profile.mCityName = city.optString(CITY_NAME_KEY);
        }
        ...
        if (user.has("address") && !user.isNull("address")) {
            ...
            profile.userAddress = Utils.get(user,"address");
        }
    ...
    public JSONObject getUpdatedUserProfileJson(String name, String contactName,
        ...
        if (!Utils.equals(userContactName, contactName)) {
            jsonObject.put("contact_person_name", contactName == null ? JSONObject.NULL : contactName);
        }
